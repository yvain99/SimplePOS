...
@section('title', 'Point Of Sales - Edit Order')
@section('content')

<div class="purchase-add-content row">
    <div class="col m7 l7 s12 z-depth-1">
        {!! Form::open(['id' => 'add-purchase-form', 'url' => 'editPurchase']) !!}
        <div class="purchase-add-header center-align">
            <input id="purchase-add-id" type="hidden" name="purchase-add-id" value="{{$purchase[0]->id}}"/>
            #<span>{{$purchase[0]->id}}</span>
        </div>
            ...
            <table id="purchase-add-products-table" class="centered" width="100%">
                <tbody>
                    ...
                    for ($i = 0; $i < count($productIds); $i++) {
                        ...
                        ?>
                        <tr id="purchase-product-{{$productIds[$i]}}-{{$productTypes[$i]}}"> 
                            <td> 
                                <input  
                                    id="purchase-product-{{$productIds[$i]}}-{{$productTypes[$i]}}-qty"  
                                    class="center-align" 
                                    type="hidden"  
                            ...
                            <td> 
                                <input  
                                    id="purchase-product-{{$productIds[$i]}}-{{$productTypes[$i]}}-price"  
                                    class="center-align" 
                                    type="hidden"  
                            ...
                            <td id="purchase-product-{{$productIds[$i]}}-{{$productTypes[$i]}}-total">
                                {{number_format($productPrices[$i] * $productQtys[$i])}}
                            </td> 
                            <td> 
                                <input  
                                    id="purchase-product-{{$productIds[$i]}}-{{$productTypes[$i]}}-supplier"  
                                    class="center-align" 
                                    type="hidden"  
                                    name="purchase-product-supplier[]"  
                                    value="{{$productSuppliers[$i]}}"/> 
                                {{$productSupplierNames[$i]}}
                            </td> 
                            <td> 
                                <input  
                                    id="purchase-product-{{$productIds[$i]}}-{{$productTypes[$i]}}-id"  
                                    class="center-align" 
                                    type="hidden"  
                                    name="purchase-product-id[]"  
                                    value="{{$productIds[$i]}}"/> 
                                <input  
                                    id="purchase-product-{{$productIds[$i]}}-{{$productTypes[$i]}}-type"  
                                    class="center-align" 
                                    type="hidden"  
                                    name="purchase-product-type[]"  
                                    value="{{$productTypes[$i]}}"/> 
                                <div class="purchase-add-product-remove-btn"  
                                     ng-click="removePurchaseAddProduct('{{$productIds[$i]}}','{{$productTypes[$i]}}');"> 
        ...
        <input id="purchase-data-total" type="hidden" value="{{$purchase[0]->total}}">
        {!! Form::close() !!}
        <span id="purchase-edit-identifier" style="display:none;"></span>
    </div>
